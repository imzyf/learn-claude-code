...
session directories (`s01_agent_loop/`, `s02_tool_use/`, ... `s20_comprehensive/`)
and are run with **tsx**. LLM calls go through the **Vercel AI SDK**
(`ai` + `@ai-sdk/anthropic`). Shared helpers live in `lib/` — `lib/model.ts`
builds the AI SDK model from `.env` (supports the Anthropic-compatible
providers listed in `.env.upstream`).
