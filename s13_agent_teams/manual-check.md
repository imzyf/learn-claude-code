...
本章的机制：一个 Lead 加若干持久队友，队友是游离的 async 循环，在 WORK 和 IDLE 之间切换；通信走文件邮箱（`.mailboxes/<agent>.jsonl`，读取即消费），任务走共享任务板（`.tasks/`），工作目录由任务的 `worktree` 绑定决定。入口用一个事件队列同时接收用户输入和 250ms 轮询发现的 Lead 收件箱事件。这份清单验八件事：spawn 前先等用户确认、队友事件怎么回流成新一轮、计划闸门拦不拦得住修改型工具、worktree 决定队友的 cwd、一个 owner 只能有一项进行中的任务加 IDLE 自己找活、队友那侧不读终端问权限、关机协议的往返、以及提示符与退出。最后一节记一个会烧钱的已知失败模式和它的中止办法。

## 怎么读这份清单

- 反引号里的英文是逐字粘给 Lead 的输入，下一行的中文只是它的意思，不要输入中文那一行。
- 「期望」后面的 plaintext 块是终端输出或 `tool_result` 原文。`task_xxxxxxxx`、`req_123456` 是随机 ID，比对时看句式不看数字。
- 任务的 `subject` 由模型自己写，清单里写的 subject 是示例。判据落在工具名、状态和错误句式上，不要求 subject 逐字一致。

## 场景

八节共用一个场景，内容全部钉死，好让每一步都能用一条命令判对错：

| 任务 | 产物 | 内容 | 依赖 |
| --- | --- | --- | --- |
| A | `.tmp/s13-check/config.json` | 恰好 `{"name": "s13-check", "version": "1.0.0"}` | 无 |
| B | `.tmp/s13-check/README.md` | 首行 `# s13-check` | 无 |
| C | 无（只跑 bash 校验） | 读 A 的 `name` 字段、读 B 的首行 | A、B |

内容写死是为了让 C 有可判的标准，也让重跑之间可比。内容自由发挥时，C 只能验「文件存在」，这一节就白验了。
